<script lang="ts">
	...
	let last_message_widths: number[] = Array(messages.length).fill(160);
	let last_message_heights: number[] = Array(messages.length).fill(0);
	...
	$: if (in_edit_mode && !previous_edit_mode) {
		...
		for (let idx = offset; idx < messageElements.length; idx++) {
			if (idx >= 0) {
				last_message_widths[idx - offset] = messageElements[idx]?.clientWidth;
				last_message_heights[idx - offset] = messageElements[idx]?.clientHeight;
			}
		}
	}
			...
			{#each messages as message, thought_index}
				...
				>
					{#if in_edit_mode}
						...
						<textarea
							class="edit-textarea"
							style:width={`max(${last_message_widths[thought_index]}px, 160px)`}
							style:min-height={`${last_message_heights[thought_index]}px`}
							autofocus
